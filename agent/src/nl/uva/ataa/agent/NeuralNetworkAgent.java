...
public abstract class NeuralNetworkAgent implements AgentInterface {
     ...
     * 
     * @param chromosome
     *            The chromosome which contains the weights
     */
    public void setWeights(final IChromosome chromosome) {
        this.setWeights(convertChromosomeToWeights(chromosome));
    }

    /**
     * Convert a given chromosome to a weight array
     * 
     * @param chromosome
     *            The given chromosome
     * @return A double array containing the weights
     */
    private static double[] convertChromosomeToWeights(final IChromosome chromosome) {
        double[] weights = new double[chromosome.getGenes().length];

        for (int i = 0; i < chromosome.getGenes().length; ++i) {
            SuperGene superGene = (SuperGene) chromosome.getGene(i);
