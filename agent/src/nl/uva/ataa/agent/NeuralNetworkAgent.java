...
public abstract class NeuralNetworkAgent extends Chromosome implements
	...
	/**
	 * Adds a layer to the neural network and adds a bias neuron to the previous
	 * layer which connects to every neuron in the new layer.
	 * 
	 * @param neuralNetwork
	 ...
	 */
	protected void addLayerWithBias(final int numNeurons,
			final InputFunction inputFunction,
			final TransferFunction transferFunction) {
		final Neuron bias = new BiasNeuron();
		mNeuralNetwork.getLayerAt(mNeuralNetwork.getLayersCount() - 1)
				.addNeuron(bias);

		final Layer layer = getLayer(numNeurons, inputFunction,
				transferFunction);
	 ...
	 *            The transfer function to use for the neurons in the new layer
	 */
	protected void addLayer(final int numNeurons,
			final InputFunction inputFunction,
			final TransferFunction transferFunction) {
		mNeuralNetwork.addLayer(getLayer(numNeurons, inputFunction,
				transferFunction));
	}
	 ...
	 *            The transfer function to use for the neurons in the new layer
	 */
	protected Layer getLayer(final int numNeurons,
			final InputFunction inputFunction,
			final TransferFunction transferFunction) {
	 ...
	 */
	protected void connectNeurons(final int inputLayer, final int inputNeuron,
			final int outputLayer, final int outputNeuron) {
		final Neuron input = mNeuralNetwork.getLayerAt(inputLayer).getNeuronAt(
				inputNeuron);
		final Neuron output = mNeuralNetwork.getLayerAt(outputLayer)
				.getNeuronAt(outputNeuron);
