...
public class HelicopterState {
    ...
    static final double MAX_VEL = 5.0, // m/s
            MAX_POS = 20.0, MAX_RATE = 2 * 3.1415 * 2, MAX_QUAT = 1.0, MIN_QW_BEFORE_HITTING_TERMINAL_STATE = Math
                    .cos(30.0 / 2.0 * Math.PI / 180.0), MAX_ACTION = 1.0;
    ...
    public void stateUpdate(final Action a) {
        ...
        for (int t = 0; t < (int) (DT_CONTROL / dt); ++t) {
            ...
            HeliVector uvw_force_from_heli_over_m = new HeliVector(
                    -heli_model_u_drag * (uvw.x + wind_uvw.x) + noise[0], -heli_model_v_drag * (uvw.y + wind_uvw.y)
                            + heli_model_tail_rotor_side_thrust + noise[1], -heli_model_w_drag * uvw.z
                            + heli_model_u3_w * a.doubleArray[3] + noise[2]);
            ...
            // *** new orientation ***
            // ----------------------------------------------------------------------
            HeliVector axis_rotation = new HeliVector(this.angular_rate.x * dt, this.angular_rate.y * dt,
            ...
            // mayday, mayday...
            if (!env_terminal
                    && (Math.abs(this.position.x) > MAX_POS || Math.abs(this.position.y) > MAX_POS
                            || Math.abs(this.position.y) > MAX_POS || Math.abs(this.velocity.x) > MAX_VEL
                            || Math.abs(this.velocity.y) > MAX_VEL || Math.abs(this.velocity.z) > MAX_VEL
                            || Math.abs(this.angular_rate.x) > MAX_RATE || Math.abs(this.angular_rate.y) > MAX_RATE
                            || Math.abs(this.angular_rate.z) > MAX_RATE || Math.abs(this.q.w) < MIN_QW_BEFORE_HITTING_TERMINAL_STATE))
                env_terminal = true;
        }
